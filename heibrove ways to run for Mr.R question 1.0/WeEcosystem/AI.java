...
public class AI  
{
   ...
   static AbstOrganism runAway = null;
    ...
    public static void checkPredatorHerb(Herbivore herb) { // Only used for Herb.
        if (herb.givesOffListPredator().isEmpty() || herb == null) {
            herb.shift(); // if no dangerous, shift like usual
            return;
        } else {
            herb.thinkNaively(); // decide the direction
            //herb.thinkElderly();
            herb.move(herb.speed); // move along
        }
    }
